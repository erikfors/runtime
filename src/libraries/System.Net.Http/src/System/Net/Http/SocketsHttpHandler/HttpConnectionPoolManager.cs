        ...
        private async ValueTask<HttpConnectionKey> GetConnectionKeyAsync(HttpRequestMessage request, Uri? proxyUri, bool isProxyConnect, CancellationToken cancellationToken)
        {
            ...
            if (proxyUri != null)
            {
                ...
                if (HttpUtilities.IsSocksScheme(proxyUri.Scheme))
                {
                    ...
                    if (sslHostName != null)
                    {
                        return new HttpConnectionKey(HttpConnectionKind.SslSocksTunnel, uri.IdnHost, uri.Port, sslHostName, proxyUri, identity, customKey);
                    }
                    else
                    {
                        return new HttpConnectionKey(HttpConnectionKind.SocksTunnel, uri.IdnHost, uri.Port, null, proxyUri, identity, customKey);
                    }
                }
